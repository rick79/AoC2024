...
function walk_the_line(y::Int64, x::Int64, Δy::Int64, Δx::Int64, map::Matrix{Char})
...
function part_one(map::Matrix{Char})
    ...
    res = walk_the_line(pos[1], pos[2], -1, 0, map)
    println("Part One: $(length(res))")
end

function part_two(map::Matrix{Char})
    pos = findfirst(f->f=='^', map)
    path = walk_the_line(pos[1], pos[2], -1, 0, map)
    counter = 0
    for (y, x) ∈ path
        if all_over_again(pos[1], pos[2], -1, 0, map, y, x)
            counter += 1
        end
    end
...
#map = read_data("Day06-test.txt")
map = read_data("Day06-data.txt")
part_one(map)
part_two(map)