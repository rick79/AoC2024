...
function part_one(m::Matrix{Char})
    ...
    (ye, xe) = ci2t(findfirst(f->f=='E', m))
    (_, prev) = dijkstra(ys, xs,m)
    path = get_path(ys, xs, ye, xe, prev)
    counter = sum([i2 - manhattan(y1, x1, y2, x2) >= 100 for (i1, (y1, x1)) ∈ enumerate(path) for (i2, (y2, x2)) ∈ enumerate(path[i1+1:end]) if manhattan(y1, x1, y2, x2) <= 2])
...
function part_two(m::Matrix{Char})
    ...
    (_, prev) = dijkstra(ys, xs,m)
    path = get_path(ys, xs, ye, xe, prev)
    counter = sum([(i2 - manhattan(y1, x1, y2, x2)) >= 100 for (i1, (y1, x1)) ∈ enumerate(path) for (i2, (y2, x2)) ∈ enumerate(path[i1+1:end]) if manhattan(y1, x1, y2, x2) <= 20])
    println("Part two: $counter")
end

#data = read_data("./Day20/test.txt")
data = read_data("./Day20/data.txt")
