...
ules that involve missing page numbers are not used.
...
function read_data(path::String)
    parts = split(open(path) do file read(file, String) end, "\n\n")
    rules = [Tuple(parse.(Int64, split(n, "|"))) for n ∈ eachsplit(parts[1], "\n")]
    updates = [parse.(Int64, split(n, ",")) for n ∈ eachsplit(parts[2], "\n")]
    return (updates, rules)
end

function check_rules(page::Int64, pages::Vector{Int64}, rules::Vector{Tuple{Int64, Int64}})
...
function uglysort(pages::Vector{Int64}, rules::Vector{Tuple{Int64, Int64}})
    sortedpages = Vector{Int64}()
    for page ∈ pages
...
#(updates, rules) = data = read_data("Day05-test.txt")
(updates, rules) = data = read_data("Day05-data.txt")
